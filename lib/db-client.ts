...
// Auth methods
const auth = {
  signUp: async (params: { email: string; password: string; options?: { data: { first_name: string; last_name: string } } }): Promise<{ data: { user: null; session: null } | null; error: { message: string } | null }> => {
    try {
  ...
  signIn: async (params: { email: string; password: string }): Promise<{ data: any; error: any }> => {
    try {
  ...
  getSession: async (): Promise<{ data: { session: { access_token: string; user: any } | null }; error: { message: string } | null }> => {
    try {
      const token = Cookies.get('auth-token');
  ...
  verifyOtp: async (params: { token_hash: string; type: 'signup' | 'recovery' }): Promise<{ data: any; error: any }> => {
    try {
  ...
  resetPasswordForEmail: async (email: string): Promise<{ data: any; error: any }> => {
    try {
  ...
  updateUser: async (params: { data: { password?: string } }): Promise<{ data: any; error: any }> => {
    try {
      const { data } = params;
...
const from = (table: string) => {
  return {
    select: (columns = '*') => {
      return {
        eq: async (column: string, value: any): Promise<{ data: any; error: any }> => {
          try {
        ...
        order: (column: string, options: { ascending?: boolean } = {}) => {
          ...
          return {
            limit: async (limit: number): Promise<{ data: any; error: any }> => {
              try {
    ...
    insert: async (values: any): Promise<{ data: any; error: any }> => {
      try {
...
const storage = {
  from: (bucket: string) => {
    return {
      upload: async (path: string, file: File): Promise<{ data: any; error: any }> => {
        try {
...
export function createServerDbClient(cookieString: string) {
  ...
  const serverFetch = async (endpoint: string, options: RequestInit = {}): Promise<Response> => {
    const headers = {
      ...options.headers,
      'Authorization': token ? `Bearer ${token}` : '',
      'Content-Type': 'application/json',
  ...
  // Server-side auth
  const serverAuth = {
    getUser: async (): Promise<{ data: any; error: any }> => {
      try {
